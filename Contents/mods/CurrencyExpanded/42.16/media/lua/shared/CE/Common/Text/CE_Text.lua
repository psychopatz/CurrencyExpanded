...
CurrencyExpanded.Text = CurrencyExpanded.Text or {}

local Text = CurrencyExpanded.Text
...
local function getTableName(domain, lang)
    return "CE_Text_" .. tostring(domain) .. "_" .. tostring(lang)
end
...
function Text.LogMissing(key)
    ...
    if not (CurrencyExpanded.Config and CurrencyExpanded.Config.DebugText == true) then
        return false
    end
...
function Text.GetList(prefix, fallbackList, maxCount)
    local normalized = tostring(prefix or "")
    local language = getLanguageCode()
    local values = {}
    local limit = math.max(1, math.floor(tonumber(maxCount) or 64))

    for index = 1, limit do
        local key = normalized .. "_" .. tostring(index)
        local resolved = resolveGameText(key)
        if not resolved then
            resolved = getFallbackValue(key, language)
        end
    ...
    if type(fallbackList) == "table" then
        return fallbackList
    end

    Text.LogMissing(normalized .. "_1")
    return {}
end
